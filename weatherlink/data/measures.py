...
celsius = lambda x: 'NA' if x == 255 else round(((float(x) / 10) - 32) / 1.8, 1)
mps = lambda x: round(float(x) / 0.44704, 1)
mm = lambda x: round(float(x) / 100 / 0.039370, 1)
mbar = lambda x: round((float(x) / 1000) * 33.863753, 1)


def decompress_date_europe(t):
    return "20%d-%02d-%02d" % (t >> 9, (t >> 5) % 16, t % 32)


format_map['EU'] = {
    "date": decompress_date_europe,
    "time": decompress_time,
    "out_temp": celsius,
    "hi_out_temp": celsius,
    "low_out_temp": celsius,
    "rainfall": mm,
    "hi_rain_rate": mm,
    "barometer": mbar,
    "solar_rad": nothing,
